...
test('fingerprint-oss systemInfo Test', async ({ page }) => {
  ...
  expect(testData.systemInfo.incognito.browserName).toBe('Chrome'); // Ensure it's using Chrome
  ...
  expect(testData.systemInfo.timezone).toBe('Asia/Calcutta'); // Ensure timezone is correct
